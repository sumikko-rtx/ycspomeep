...
#/* other older python versions */
else:
    import cgi
    
    def __my_html_escape(text):
        return cgi.escape(text, quote=True).replace(u'\n', u'<br />').replace(u'\t', u'&emsp;').replace(u'  ', u' &nbsp;')

    html_escape = __my_html_escape
...
def email_report2(subject='important messages from the backup server',
    ...
    _body.append(
        '<p>This mail was sent by ycspomeep version {0}.</p>'.format(html_escape(CURRENT_VERSION)))
